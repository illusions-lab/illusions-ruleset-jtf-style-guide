/**
 * jtf-2-1-8 — 全角数字を半角数字へ統一
 *
 * JTF日本語標準スタイルガイド 2.1.8 は算用数字を半角で表記するよう定める。
 * 本ルールは U+FF10-U+FF19 の全角数字を検出し、対応する半角数字への置換を提案する。
 */
...
export function createJtfNumericStandard(ctx: RulesetContext, manifest: RulesetManifest): LintRule {
  const meta = manifest.rules.find((r) => r.ruleId === "jtf-2-1-8");
  if (!meta) throw new Error("manifest is missing the jtf-2-1-8 rule");
